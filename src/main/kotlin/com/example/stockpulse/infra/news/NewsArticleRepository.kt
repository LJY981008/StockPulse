...
class NewsArticleRepository(
    ...
    override fun findByCompanyOrderByPublishedAtDesc(company: Company, page: Int, size: Int): List<NewsArticle> {
        val jpaCompany = companyJpaRepository.findByStockCode(company.stockCode) 
            ?: return emptyList()
    ...
    override fun findByDataSourceOrderByPublishedAtDesc(dataSource: DataSource, page: Int, size: Int): List<NewsArticle> {
        val jpaDataSource = dataSourceJpaRepository.findBySourceName(dataSource.sourceName)
            ?: return emptyList()
    ...
    override fun findByCompanyAndPublishedAtBetweenOrderByPublishedAtDesc(
        ...
        return newsArticleJpaRepository.findByJpaCompanyAndPublishedAtBetweenOrderByPublishedAtDesc(
            jpaCompany, startDate, endDate, pageable
        ).content
            .map { it.toNewsArticle(company, it.jpaDataSource.toDataSource()) }
    }
    ...
    override fun findByOriginalUrl(originalUrl: String): NewsArticle? {
        val jpaNewsArticle = newsArticleJpaRepository.findByOriginalUrl(originalUrl)
            ?: return null
    ...
    override fun save(newsArticle: NewsArticle): NewsArticle {
        val jpaCompany = companyJpaRepository.findByStockCode(newsArticle.company.stockCode)
            ?: throw IllegalArgumentException("Company not found: ${newsArticle.company.stockCode}")
        
        val jpaDataSource = dataSourceJpaRepository.findBySourceName(newsArticle.dataSource.sourceName)
            ?: throw IllegalArgumentException("DataSource not found: ${newsArticle.dataSource.sourceName}")
        
        val savedJpaNewsArticle = newsArticleJpaRepository.save(
    ...
    override fun findById(articleId: UUID): NewsArticle? {
        val jpaNewsArticle = newsArticleJpaRepository.findById(articleId)
    ...
    override fun saveAll(newsArticles: List<NewsArticle>): List<NewsArticle> {
        val savedArticles = newsArticles.map { newsArticle ->
            val jpaCompany = companyJpaRepository.findByStockCode(newsArticle.company.stockCode)
                ?: throw IllegalArgumentException("Company not found: ${newsArticle.company.stockCode}")
